function chkTxt() {
    ...
    // 서버에 비밀번호 확인 요청 보내기
    axios.post("http://localhost:3000/user/passwordTest", { userid, pw })
      .then(response => {
        // 비밀번호가 일치하는 경우
...
function chktoCanEdit() {
  ...
  const id = document.getElementById('id');
    ...
    if (hiddenDiv.classList.contains('hidden')) {
      ...
      axios
      .post("http://localhost:3000/user/checkDuplicate", {id})
          .then((response) => {
          if (response.data.isDuplicate) {
              ...
              axios.post("http://localhost:3000/user/changeUserId", {userid, id})
              .then(response => {
